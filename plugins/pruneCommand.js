...
const PruneUtil = require('../util/pruneUtil');
const { senderIsOwner } = require('../util/botUtil');
const { prefix } = require('../config');


function respond (msg, client) {
	const { channel, guild, member: sender } = msg;
	if (msg.isPrivate || !senderIsOwner(msg)) return;
	...
	const { deleteMessages, fetchMoreMessages, fetchMoreSpecificMessages }
		= new PruneUtil(client);

	acs('prune', a => {
		// Sender can't delete or pin messages
		if (!sender.can(Permissions.Text.MANAGE_MESSAGES, guild)) return;
